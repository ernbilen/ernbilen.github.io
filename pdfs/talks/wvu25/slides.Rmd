...
date: "WVU Economics Seminar, Feb 21, 2025"
output:
...
Conflict between Israel-Palestine, Oct 2023-cont.

\>50,000 casualties (military and civilian)

In progress! Indirect effects: players from the Arab League nations

```{r out.width='100%', fig.height=4, eval=require('leaflet')}
library(leaflet)
leaflet() %>% addTiles() %>% setView(35.1, 35.5, zoom = 5)
```
...
- Conflicts impact the quality of moves made by chess players, but little evidence for "aggressive" games played.
- Average quality is better, frequency of mistakes are higher.
  - Potential channels: Higher effort and/or high emotional response to conflict.
- Games end up more as draws.
- More resiliency: longer games with less resigning and more mates.
- Impact is not uniform:
  - Lowest rated players show more aggression. Quality improvements come from the highest rating quartile.
  - More aggression in longer time control: rapid.
  - About uniform response from both sides.
- Nationwide public support on conflicts.
